...
for item in items:
    ...
    try:
        ...
        calc_items.append(requests.get("https://esi.evetech.net/latest/universe/types/{}".format(item)).json().get("name"))
        growths.append((predicted_price-metrics["lowest_sell_price"])/metrics["lowest_sell_price"])
        prices.append(predicted_price)

        count += 1
        if count % 10 == 0:
            df = pd.DataFrame({"item":calc_items, "growth":growths, "price":prices})
